...
from mcmodels.core.utils import (
    get_structure_id,
    get_ordered_summary_structures,
    get_leaves_ontologicalorder,
    get_indices_2ormore,
    get_indices,
    get_indices2,
    get_eval_indices,
    screen_index_matrices,
    screen_index_matrices2,
...
from mcmodels.models.voxel.crossvalidation import (
    get_nwloocv_predictions_multimodel_merge_dists,
)
...
def get_row_col_names(connectivity_data, target_ordering):
    ...
    rnames = np.asarray(
        [ia_map[target_ordering[i]] for i in range(len(target_ordering))]
    )
    ...
    for iy in target_ordering:
        ipsi_indices = np.concatenate(
            [ipsi_indices, np.where(ipsi_target_regions == iy)[0]]
        )
        contra_indices = np.concatenate(
            [contra_indices, np.where(contra_target_regions == iy)[0]]
        )
    ...
    ccomb = np.vstack(
        [
            np.concatenate(
                [
                    np.repeat("ipsi", connectivity_data.ipsi_target_regions.shape[0]),
                    np.repeat("contra", connectivity_data.contra_target_regions.shape),
                ]
            ),
            cnames,
        ]
...
ontological_order_leaves_summary = get_aligned_ids(
    st, ontological_order_leaves, ontological_order
)
...
connectivity_data.summary_structures = {
    sid: connectivity_data.structure_datas[sid].summary_structures
    for sid in major_structure_ids
}  # get_indices_2ormore(connectivity_data.leafs)
...
contra_targetkey = connectivity_data.structure_datas[sid0].projection_mask.get_key(
    structure_ids=targ_ord, hemisphere_id=1
)
...
ipsi_targetkey = connectivity_data.structure_datas[sid0].projection_mask.get_key(
    structure_ids=targ_ord, hemisphere_id=2
)
connectivity_data.get_regionalized_normalized_data(
    source_ord, ipsi_targetkey, contra_targetkey
)
summary_structures = {
    sid: connectivity_data.structure_datas[sid].summary_structures
    for sid in major_structure_ids
...
with open("analyses/results/EL_leafsurface_060622_leafleaf.pickle", "rb") as handle:
