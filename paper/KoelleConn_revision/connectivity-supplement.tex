...
The arguments of this normalization process - injection signals $x(i)$, projection signals $y(i)$, injection fraction $F(i)$, and data quality mask $q(i)$ - were downloaded using the Allen SDK, a programatic interface to the brain connectivity data. %and injection fraction $F(i)$ 
...
%We define $x \odot x' = x(l) x'(l)$ for all $l \in \mathcal B$.
Denote $m(i)$ as the major structure containing experiment $i$, and naturally defining $\odot$ for maps $\mathcal B \to [0,1]$ by e.g.
 $( y(i) \odot q(i)) |_l := ( y(i)  |_l) ( q(i) |_l )$, we then can write the preprocessing algorithm.
...
By this definition, similar cell classes have relatively small Cre-distance.
