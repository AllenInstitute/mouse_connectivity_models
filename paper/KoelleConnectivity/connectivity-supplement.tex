...
\begin{algorithmic}
\caption{{\bf Input} Injection $x(i)$, Projection $y(i)$, Injection centroid $c(i) \in \mathbb R^3$, injection fraction $F(i)$, data mask $M(i)$}
\State Data-quality censor $y_M (i) = M(i) \odot y(i)$ 
\State Average structures $y_S (i) = A y_M(i)$
\State Normalize $\tilde y(i) = \frac{y_S (i)}{\|y_S (i)\|}$
%\Return $\tilde y(i)$
%\caption{(Local dataset $\delta_i^{x} $, local embedding coordinates $\delta_i^{\phi}, \tsalg$) ) }
%\begin{algorithmic}[1]
%\State { known at data points, embedding algorithm \embed~ and algorithm parameters, \mmp{weights?}, embedding dimension $m$}
%\State Compute neighborhood graph $\neigh_i $ using (\ref{eq:kernelmatrix})% \delta_i^f$, for $i=1:n$ 
% \State Compute Gaussian kernel matrix $K$ by $K_{ij}=\exp(-\|\tilde\delta_i^{\xi_\epsilon}\|^2/h^2)$ for $j\in\neigh(i)$, and weights $w_i=\sum_{j\in \neigh(i)}K_{ij}$ for $i=1,\ldots n$, a (see Section \ref{sec:weights})
% \State Compute Gaussian kernel matrix $K$ by $K_{ij}=\exp(-\| \delta_i^{\xi_\epsilon}\|^2/h^2)$ for $j\in\neigh(i)$, and weights $w_i=\sum_{j\in \neigh(i)}K_{ij}$ for $i=1,\ldots n$, a (see Section \ref{sec:manifold_learning})
 \State {\bf Output}$\tilde y(i)$ )  % \mmp{what -- notation for grad f needed}
\end{algorithmic}
\end{algorithm}
...
%\begin{algorithmic}
%\label{alg:norm}
%\KwData{{Injection $x(i)$, Projection $y(i)$, Injection centroid $c(i) \in \mathbb R^3$, injection fraction $F(i)$, data mask $M(i)$}}
