...
def get_ccf_data(cache, experiment_id, folder):
    ...
    eid_data.data_quality_mask = cache.get_data_mask(
        experiment_id,
        folder + "experiment_" + str(experiment_id) + "/data_mask_100.nrrd",
    )[0]
    eid_data.injection_signal = cache.get_injection_density(
        experiment_id,
        folder + "experiment_" + str(experiment_id) + "/injection_density_100.nrrd",
    )[0]
    eid_data.injection_fraction = cache.get_injection_fraction(
        experiment_id,
        folder + "experiment_" + str(experiment_id) + "/injection_fraction_100.nrrd",
    )[0]
    eid_data.projection_signal = cache.get_projection_density(
        experiment_id,
        folder + "experiment_" + str(experiment_id) + "/projection_density_100.nrrd",
    )[0]
    return eid_data
...
def get_injection_hemisphere_id(injection_density, majority=False):
    ...
    if injection_density.ndim != 3:
        raise ValueError(
            "injection_density must be 3-array not (%d)-array" % injection_density.ndim
        )
...
def get_indices2(firstlist, firstlisttest, secondlist):
    ...
    for i in range(len(sl_unique)):
        output[
            i,
            np.intersect1d(
                np.where(np.isin(firstlist, firstlisttest))[0],
                np.where(secondlist == sl_unique[i])[0],
            ),
        ] = 1
    return output
...
def screen_index_matrices(model_index_matrices, eval_index_matrices):
    ...
    for m in range(nmodels):
        eval_index_matrices2[m] = screen_indices(
            model_index_matrices[m], eval_index_matrices[m]
        )
...
def get_matched_index_matrices(model_index_matrices, eval_index_matrices):
    ...
    for sid in major_structure_ids:
        ...
        matched_model_index_matrices[sid] = np.asarray(
            matched_model_index_matrices[sid], dtype=int
        )
...
def get_masked_data_volume(data_volume, data_mask, tolerance=0.0):
    ...
    if data_volume.shape != data_mask.shape:
        raise ValueError(
            "data_volume (%s) and data_mask (%s) must be the same "
...
def get_twoormore(classdict):
    ...
    for sid in major_structure_ids:
        ...
        freq = np.asarray(
            [count[classdict[sid][i]] for i in range(len(classdict[sid]))]
        )
...
def get_regionalized_normalized_data(
    msvds, cache, source_order, ipsi_key, contra_key
    ...
    for sid in major_structure_ids:
        ...
        ipsi_target_regions, ipsi_target_counts = nonzero_unique(
            ipsi_key, return_counts=True
        )
        contra_target_regions, contra_target_counts = nonzero_unique(
            contra_key, return_counts=True
        )
        ...
        projections = projections / np.expand_dims(
            np.linalg.norm(projections, axis=1), 1
        )
...
def get_ontological_order_leaf(leafs, ontological_order, st):
    ...
    for i in range(len(ontological_order)):
        ...
        if len(which_stos) > 0:
            print(i)
            ontological_order_leaf = np.append(
                ontological_order_leaf, stos[0][which_stos]
            )
        if np.isin(ontological_order[i], leaf_present):
            print(i)
            ontological_order_leaf = np.append(
                ontological_order_leaf, ontological_order[i]
            )
...
def get_connectivity(
    ...
    source_exp_countvec, source_exp_countvec_wt = get_countvec(
        source_ordering, ia_map, creline, experiments_minor_structures
    )
    ...
    for m in range(nms):
        ...
        minor_structures = source_ordering[
            np.where(np.isin(source_ordering, np.unique(leafs[sid])))
        ]
        ...
        for n in range(len(minor_structures)):
            ...
            im = Mask.from_cache(
                cache, structure_ids=[minor_structures[n]], hemisphere_id=2
            )
            weights = pairwise_kernels(
                X=msvds[sid].centroids[minor_structure_inds],
                Y=im.coordinates,
                metric="rbf",
                gamma=gamma,
                filter_params=True,
            ...
            weights[np.where(np.isnan(weights))] = 0.0
            predictions = np.dot(
                weights.transpose(),
                msvds[sid].reg_proj_vcount_norm_renorm[minor_structure_inds],
            )

            # average over source region voxels
            union_key = im.get_key(structure_ids=source_ordering, hemisphere_id=2)
            source_regions, source_counts = nonzero_unique(
                union_key, return_counts=True
            )
            prediction_union = unionize(predictions.transpose(), union_key)
            prediction_union_norms[m][
                n
            ] = prediction_union.transpose() / np.expand_dims(source_counts, 1)
    ...
    for m in range(nms):
        prediction_union_norms_ms[m] = np.vstack(
            [
                prediction_union_norms[m][n]
                for n in range(len(prediction_union_norms[m].keys()))
            ]
        )

    cd = np.vstack(
        [
            prediction_union_norms_ms[m]
            for m in range(len(prediction_union_norms_ms.keys()))
        ]
    )
    ...
    targ_ords = np.concatenate(
        [
            permutation(ipsi_target_regions),
            len(ipsi_target_regions) + permutation(contra_target_regions),
        ]
    )
    ...
    df = pd.DataFrame(
        cd[row_reorder][:, targ_ords], index=rownames, columns=np.asarray(colnames)
    )
    return df
...
def get_nw_loocv(msvd, indices, loocv, hyperparameters):
    if len(indices) > 1:
        ...
        for g in range(nhyp):
            loocv_predictions[g, indices] = loocv(
                projections[indices], centroids[indices], hyperparameters[g]
            )
...
def get_countvec(ontological_order, ia_map, creline, experiments_minor_structures):
    ...
    sourcenames = np.asarray(
        [ia_map[ontological_order[i]] for i in range(len(ontological_order))]
    )
    source_exp_counts = {}
    ...
    for i in range(len(sourcenames)):
        ...
        for sid in major_structure_ids:
            source_exp_counts[sourcenames[i]] += len(
                np.where(experiments_minor_structures[sid] == sourcenames[i])[0]
            )
...
def get_leaves_ontologicalorder(connectivity_data, ontological_order):
    ...
    levs = connectivity_data.structure_datas[
        sid0
    ].projection_mask.reference_space.structure_tree.child_ids(ontological_order)
...
def get_loss_paper(y, yhat):
    loss = (
        2
        * np.linalg.norm(y - yhat) ** 2
        / (np.linalg.norm(y) ** 2 + np.linalg.norm(yhat) ** 2)
    )
    return loss
...
def get_reduced_matrix_ninj(X, thresh, number):
    X_thresh = X.copy()
    X_thresh[np.where(X > thresh)] = 1.0
    X_thresh[np.where(X <= thresh)] = 0.0
...
def get_minorstructures(eids, data_info):
    ...
    for i in range(len(eids)):
        ...
        experiments_minors[i] = data_info["primary-injection-structure"].loc[
            experiment_id
        ]
