...
def get_embedding_cv(surface, dists, cre_distances_cv):
    ...
    for i in range(nlp):
        ...
        losses[leaf_pairs[i][0]][leaf_pairs[i][1]] = surface.predict(
            np.asarray([[d_ij, p_ij]])
        )
...
def get_loss_surface_cv_spline(projections, centroids, cres, sids, fraction):
    ...
    surface = get_surface_from_distances_spline(
        projections, centroids, cre_distances_cv, fraction
    )
    surface.cre_distances_cv = cre_distances_cv
    return surface


def get_surface_from_distances_nw(
    projections, centroids, cre_distances, fraction, gamma=100000
):
